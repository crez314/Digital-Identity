 ...
 * 예전 문구는 `same outfit`이라 레퍼런스 사진의 옷을 그대로 재현하라는 뜻이 됐다 — 운영자가
 * 프롬프트에 무대 의상을 지정해도 레퍼런스의 옷이 이겼다. 막으려던 것은 클립 중간에 옷이 바뀌는
 * 것이지 운영자가 정한 의상이 아니다.
 *
 * 계기는 레퍼런스가 전부 시스루 뷔스티에인 인물의 구간이 nsfw로 연속 거부된 일이었지만(2026-09-26),
 * 그게 원인이라는 증거는 없다 — 같은 레퍼런스로 만든 30초 클립이 그 전날 통과했다(QC 0.673).
 * 이 변경의 근거는 "운영자가 지정한 의상이 이겨야 한다"는 것이지 거부율이 아니다.
 */
